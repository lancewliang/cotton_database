...
public class GetCNBuyHistoryWeb implements DayAnt {
  ...
  private void parseList() {
    long maxDate = 0;
    try {
      maxDate = GovDaySQL.getBuyLastDay(Commodity.getCommodity("棉花"), Country.getCountry("CHN"), getSource());
    ...
    String url = "http://www.cottonchina.org/news/newsser.php?relnews=%CA%D5%B4%A2%C6%C0%CA%F6&relnews1=%CA%D5%B4%A2";
    ...
    try {
      ...
      if (!None.isEmpty(ass)) {
        ...
        for (Tag t : ass) {
          ...
          AntLogic.saveFile(getSource() + "/BuyHistory/" + year + "/", reportDateStr + ".htm", content);
        }
  ...
  private void parseDayDetail(String content) {
    try {
      ...
      long buyvalue = getData0(content);
      long totalBuyValue = getData3(content);
      ...
      govday.setBuyValue(buyvalue);
      govday.setTotalBuyValue(totalBuyValue);
  ...
  private long getData0(String str) throws Exception {
    String regex = "计划收储[0-9]+吨，实际成交[0-9]+吨";// 这就是正则表达式了
    Pattern p = Pattern.compile(regex);
    Matcher m = p.matcher(str);
    String datastr = null;
    if (m.find()) {
    ...
    if (x != -1) {
      return Long.parseLong(datastr.substring(x + 4, datastr.length() - 1));
    } else {
      throw new Exception();
    }

  }
  private long getData3(String str) throws Exception {
    String regex = "收储累计成交[0-9]+吨";// 这就是正则表达式了
    Pattern p = Pattern.compile(regex);
    Matcher m = p.matcher(str);
    String datastr = null;
    if (m.find()) {
    ...
    int x = datastr.indexOf("收储累计成交");
    if (x != -1) {
      return Long.parseLong(datastr.substring(x + 6, datastr.length() - 1));
    } else {
      throw new Exception();
    }
