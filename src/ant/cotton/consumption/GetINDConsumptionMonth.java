...
import model.entity.consumption.db.ConsumptionMonthSQL;
...
public class GetINDConsumptionMonth implements DayAnt {
  public static void main(String[] args) {
    SetENVUtil.setENV();
    GetINDConsumptionMonth mhhf = new GetINDConsumptionMonth();
    try {
      mhhf.doAnt();
    } catch (Exception e) {
  ...
  public void doAnt() {
    try {
      ...
      long lastDay = ConsumptionMonthSQL.getLastDay(Commodity.getCommodity("ÃÞ»¨"), Country.getCountry("IND"), getSource());
      if (lastDay <= 0)
        lastDay = 200810;
      Parser parser = Parser.createParser(html, "windows-1252");
      NodeList nl = parser.parse(null);
      Tag link1tag = HTMLParseUtil.getLinkBySubString(nl, "Consumption by Non-SSI Mills");
      Tag link2tag = HTMLParseUtil.getLinkBySubString(nl, "Consumption by SSI Mills");
      ...
      File f1 = AntLogic.getFile(getSource() + "/Consumption", "Consumption by Non-SSI Mills.html");
      if (f1.exists()) {
        ...
        if (!content.equals(link1html)) {
          AntLogic.saveFile(getSource() + "/Consumption", "Consumption by Non-SSI Mills.html", link1html);
        }
      } else {
        AntLogic.saveFile(getSource() + "/Consumption", "Consumption by Non-SSI Mills.html", link1html);
      }
      File f2 = AntLogic.getFile(getSource() + "/Consumption", "Consumption by SSI Mills.html");
      if (f2.exists()) {
        InputStream in = new FileInputStream(f2);
        String content = FileStreamUtil.getFileContent(in);
        in.close();
        if (!content.equals(link2html)) {
          AntLogic.saveFile(getSource() + "/Consumption", "Consumption by SSI Mills.html", link2html);
        }
      ...
      for (long reportDate : monthsints) {
        ...
        if (v1 != null && v2 != null) {
          ConsumptionMonth monthObj = ConsumptionMonthSQL.getObj(reportDate, Country.getCountry("IND"), Commodity.getCommodity("ÃÞ»¨"), getSource());
          if (monthObj == null) {
          ...
          ConsumptionMonthSQL.save(monthObj);
        }
      }
