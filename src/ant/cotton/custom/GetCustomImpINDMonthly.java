...
public class GetCustomImpINDMonthly implements DayAnt {

  public static void main(String[] args) throws Exception {
    SetENVUtil.setENV();
    GetCustomImpINDMonthly exp = new GetCustomImpINDMonthly();
    exp.doAnt();
  ...
  public void getImport() {
    String href = "http://dgciskol.gov.in/pdfs/Imsft_04.pdf";
    File file = AntLogic.getFile(getSource() + "/import", "Imsft_04.pdf");

    try {
      if (!file.exists()) {
        Util.getFile(href, file);
      }
      long lastDay = ImportExportMonthSQL.getTOCountryLastDay(Commodity.getCommodity("棉花"), Country.getCountry("IND"), getSource());
      ...
      if (fileDate > 0) {
        File newFile = new File(file.getParentFile(), "Imsft_04_" + fileDate + ".pdf");
        if (newFile.exists()) {
  ...
  public void praseContent(String str) {
    String line = "TABLE - 4     India's Import of Principal Commodities";
  ...
  private void saveMonthInfo(long reportDate, double total) throws SQLException {
    Date now = new Date();
    Country fromCountry = Country.getCountry("WHOLE");
    Country toCountry = Country.getCountry("IND");
